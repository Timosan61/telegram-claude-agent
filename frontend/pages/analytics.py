...
def show_analytics_page():
    ...
    if not is_connected:
        ...
        st.info("Показываем демо-данные для демонстрации функционала")
...
def show_demo_analysis_results():
    """Показать демо-результаты анализа"""
    st.info("🎭 Демонстрационные данные - показывает как выглядят результаты анализа")
    
    # Симуляция выбора анализа
    st.selectbox(
        "Выберите анализ для просмотра:",
        ["✅ @demo_channel - 1,247 сообщений", "✅ @test_news - 892 сообщения", "🔄 @crypto_chat - обрабатывается..."],
        index=0
    )
    ...
    st.write("### 📊 Активность по времени")
    demo_hours = list(range(24))
    demo_activity = [5, 3, 1, 0, 0, 2, 8, 15, 25, 30, 45, 50, 55, 48, 40, 35, 38, 42, 35, 28, 20, 15, 10, 7]
    
    fig = px.bar(x=demo_hours, y=demo_activity, title="Активность по часам (демо)")
    fig.update_layout(showlegend=False, xaxis_title="Час", yaxis_title="Количество сообщений")
    st.plotly_chart(fig, use_container_width=True)
    
    # Демо топ участников
    st.write("### 👥 Топ участники")
    demo_participants = pd.DataFrame({
        "Участник": ["Александр П.", "Maria K.", "Telegram Bot", "Анна С.", "John D."],
        "Сообщений": [156, 143, 128, 89, 67],
        "Тип": ["👤", "👤", "🤖", "👤", "👤"]
    })
    st.dataframe(demo_participants, use_container_width=True)
    
    # Демо ключевые слова
    st.write("### 🔤 Популярные слова")
    col1, col2 = st.columns(2)
    
    with col1:
        demo_words = ["привет", "спасибо", "хорошо", "сегодня", "можно", "нужно", "делать", "время"]
        demo_counts = [89, 67, 54, 43, 38, 32, 28, 24]
        words_df = pd.DataFrame({"Слово": demo_words, "Количество": demo_counts})
        
        fig_words = px.bar(words_df, x="Количество", y="Слово", orientation="h", title="Топ слова")
        fig_words.update_layout(yaxis={'categoryorder':'total ascending'})
        st.plotly_chart(fig_words, use_container_width=True)
    ...
    with col2:
        ...
        if st.button("📥 Скачать CSV"):
            st.success("✅ В реальном режиме здесь будет скачивание CSV файла")
        if st.button("📥 Скачать JSON"):
            st.success("✅ В реальном режиме здесь будет скачивание JSON файла")