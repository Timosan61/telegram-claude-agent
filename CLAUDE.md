...
# - GitHub Actions (если настроен)
# - Heroku logs (для Heroku)
# - Vercel/Netlify dashboard (для Streamlit)
```
...
6. **Wait** → ОБЯЗАТЕЛЬНО ожидать 60 секунд деплоя
7. **Check logs** → Проверить логи деплоя и убедиться в успешности
