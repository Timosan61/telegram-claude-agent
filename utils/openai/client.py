...
class OpenAIClient:
    ...
    def __init__(self):
        ...
        try:
            ...
            print("✅ OpenAI client успешно инициализирован (стандартно)")
        except Exception as e1:
            print(f"⚠️ Стандартная инициализация не удалась: {e1}")
            try:
                # Попытка 2: Минимальная инициализация
                self.client = OpenAI()
                self.client.api_key = self.api_key
                print("✅ OpenAI client успешно инициализирован (минимально)")
            except Exception as e2:
                print(f"⚠️ Минимальная инициализация не удалась: {e2}")
                try:
                    # Попытка 3: Через переменную окружения 
                    print("🔄 Попытка инициализации через переменную окружения...")
                    os.environ['OPENAI_API_KEY'] = self.api_key
                    self.client = OpenAI()
                    print("✅ OpenAI client успешно инициализирован (через env)")
                except Exception as e3:
                    print(f"❌ Все способы инициализации не удались: {e3}")
                    raise ValueError(f"Не удалось инициализировать OpenAI клиента: {e1}, {e2}, {e3}")
