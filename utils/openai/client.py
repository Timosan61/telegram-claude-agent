...
class OpenAIClient:
    ...
    def __init__(self):
        ...
        # Множественные попытки инициализации с разными параметрами
        initialization_attempts = [
            lambda: OpenAI(api_key=self.api_key),  # Простейший способ
            lambda: OpenAI(**{"api_key": self.api_key}),  # Через словарь
        ]
        ...
        for i, attempt in enumerate(initialization_attempts, 1):
            try:
                print(f"🔄 Попытка инициализации #{i}...")
                self.client = attempt()
        ...
        if self.client is None:
            print(f"❌ Все попытки инициализации неудачны")
            raise ValueError(f"Не удалось инициализировать OpenAI клиента: {last_error}")
